...
const DataJabatan = () => {
    ...
    const paginationItems = () => {
        ...
        if (startPage > 2) {
            items.unshift(
                <p key="start-ellipsis" className="py-2 px-4 border border-gray-2 dark:bg-transparent text-black font-medium bg-gray dark:border-strokedark dark:text-white">...</p>
            );
        }

        if (endPage < totalPages - 1) {
            items.push(
                <p key="end-ellipsis" className="py-2 px-4 border border-gray-2 dark:bg-transparent text-black font-medium bg-gray dark:border-strokedark dark:text-white">...</p>
            );
        }
                    ...
                    <table className='w-full table-auto'>
                        <thead>
                            <tr className='bg-gray-2 text-left dark:bg-meta-4'>
                                <th className='py-4 px-4 font-medium text-black dark:text-white'>{t('column_number')}</th>
                                <th className='py-4 px-4 font-medium text-black dark:text-white'>{t('column_position')}</th>
                                <th className='py-4 px-4 font-medium text-black dark:text-white'>{t('column_base_salary')}</th>
                                <th className='py-4 px-4 font-medium text-black dark:text-white'>{t('column_transport_allowance')}</th>
                                <th className='py-4 px-4 font-medium text-black dark:text-white'>{t('column_meal_allowance')}</th>
                                <th className='py-4 px-4 font-medium text-black dark:text-white'>{t('column_action')}</th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                <tr key={data.id}>
                                    ...
                                    <td className='border-b border-[#eee] py-5 px-4 dark:border-strokedark'>
                                        <div className='flex items-center space-x-3.5'>
                                            <Link className='hover:text-black' to={`/data-jabatan/form-data-jabatan/edit/${data.id}`}>
                                                <FaRegEdit className="text-primary text-xl hover:text-black dark:hover:text-white" />
                                            </Link>
                                            <button onClick={() => onDeleteJabatan(data.id)} className='hover:text-black'>
