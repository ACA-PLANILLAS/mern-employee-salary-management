...
export const createDataKehadiran =
  (dataPegawai, dataKehadiran, navigate) => async (dispatch) => {
    try {
      for (let i = 0; i < dataPegawai.length; i++) {
        ...
        if (!isNamaAda) {
          const response = await axios.post(`${API_URL}/data_kehadiran`, {
...
export const deleteDataKehadiran = (id) => {
  return async (dispatch) => {
    try {
      const response = await axios.delete(`${API_URL}/data_kehadiran/${id}`);
      if (response.status === 200) {
        dispatch({
